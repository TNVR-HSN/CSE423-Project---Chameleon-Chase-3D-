...
class Prop:
    ...
    NEIGHBORHOOD = [(dx, dy) for dx in range(-45, 46) for dy in range(-45, 46)]
    all = {(x, y)
           for x in range(-GRID_LENGTH + 50, GRID_LENGTH - 50 + 1)
    ...
    def generate_prop(cls):
        ...
        existing = cls.props["chair"] + cls.props["table"] + cls.props["lamp"]
        for prp in existing:
            x, y, z = prp.pos
        ...
        cls.available = set(cls.all - exclude)

        for tname in cls.types:
            max_count = 10
            while len(cls.props[tname]) < max_count and len(cls.available) > 0:
                pos = choice(list(cls.available))
                temp = (pos[0], pos[1], 0)
                cls.props[tname].append(cls(tname, temp))
    ...
    @classmethod
    def draw_chair(cls, pos, t=0, highlight=False):
        if highlight:
        ...
        for mul in (1, 3, 7, 5):
            ...
            glTranslatef(*pos)
            glRotatef(t, 0, 0, 1)
            glTranslatef(45 * cos(2 * pi * 0.125 * mul + t),
                         45 * sin(2 * pi * 0.125 * mul + t), 0)
    ...
    @classmethod
    def draw_table(cls, pos, t=0, highlight=False):
        if highlight:
            glColor3f(1.0, 0.0, 0.0)
        ...
        for mul in (1, 3, 7, 5):
            ...
            glTranslatef(*pos)
            glRotatef(t, 0, 0, 1)
            glTranslatef(k * cos(2 * pi * 0.125 * mul + t),
                         k * sin(2 * pi * 0.125 * mul + t), 0)
            gluCylinder(gluNewQuadric(), 10, 10, 80, 10, 10)
            glPopMatrix()
    ...
    @classmethod
    def draw_lamp(cls, pos, t=0, highlight=False):
        if highlight:
            glColor3f(1.0, 0.0, 0.0)
        # pointer
        glPushMatrix()
    ...
    def draw_prop(cls):
        ...
        for tname in cls.types:
            for prp in cls.props[tname]:
                ...
                t = cls.angle if prp.player else prp.angle

                if tname == "chair":
                    cls.draw_chair(prp.pos, t, highlight)
                elif tname == "table":
                    cls.draw_table(prp.pos, t, highlight)
                else:
                    cls.draw_lamp(prp.pos, t, highlight)
        ...
        if state == GLUT_DOWN:
            if button == GLUT_LEFT_BUTTON:
                ...
                if (not cls.decoy[0]) or now - cls.decoy[1] > 3:
                    new_player_pos = (player.pos[0] + dx, player.pos[1] + dy, player.pos[2])
                    ...
                    for prp in temp:
                        new_pos = (prp.pos[0] + dx, prp.pos[1] + dy, prp.pos[2])
...
def draw_splash():
    draw_tricolor_bg()
    text = "Chameleon Chase 3D"
    font = GLUT_BITMAP_HELVETICA_18

    tw = 0
    for ch in text:
        tw += glutBitmapWidth(font, ord(ch))

    x = (width - tw) // 2
    y = height // 2 - 10

    draw_text(x, y, text, font=font)
...
def draw_menu():
    ...
    draw_text((width - tw) // 2, int(height * 0.70), text, font=font)
...
def draw_diff():
    ...
    draw_text((width - tw) // 2, int(height * 0.80), text, font=font)
...
def would_cross(px, py, nx, ny, w):
    ...
    if w['o'] == 'h':
        ...
        return ((x1 - PLAYER_RADIUS <= nx <= x2 + PLAYER_RADIUS) and
                (y - half - PLAYER_RADIUS <= ny <= y + half + PLAYER_RADIUS))
        ...
        return ((y1 - PLAYER_RADIUS <= ny <= y2 + PLAYER_RADIUS) and
                (x - half - PLAYER_RADIUS <= nx <= x + half + PLAYER_RADIUS))
...
def try_ping():
    ...
    if distance_xy((hx, hy), (px, py)) <= ping_radius:
        prop_reveal_until = time.time() + 1.0


def display():
    global state, splash_start, round_over, winner_text
    global MOVE_SPEED, dash_active, winner_printed, time_frozen_at
